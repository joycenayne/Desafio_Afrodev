...
def sports_participants(data):
    ...
    print(f"Os esportes parcipantes das Olímpiadas, foram: {list_sport_format}")
...
def total_participants_by_sports(data):
    ...
    print(f"A seguir, a quantidade de participantes por esportes: {list_par_format}")
...
def sports_more_women(data):
    ...
    print(f"Os esportes com mais mulheres, são: {sports_fem_format}")

def sports_more_men(data):
    sports_female = sports_by_gender(data, "Male", "Female")
    sports_fem_format = funcs.format_string(sports_female)
    print(f"Os esportes com mais homens, são: {sports_fem_format}")
